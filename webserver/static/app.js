document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderControlsAndData() {
        ...
        nodeSelect.innerHTML = '<option value="all">All</option>' + nodes.map(n => `<option value="${n}">${n}</option>`).join('');
        logTypeSelect.innerHTML = '<option value="all">All</option>' + Array.from(logTypes).map(t => `<option value="${t}">${t}</option>`).join('');
    ...
    function renderHistogram() {
        ...
        Object.entries(jsonData.nodes).forEach(([node, nodeData]) => {
            ...
            Object.entries(nodeData).forEach(([logType, logTypeData]) => {
                ...
                Object.entries(logTypeData.logMessages || {}).forEach(([msg, msgStats]) => {
                    Object.entries(msgStats.histogram || {}).forEach(([bucket, count]) => {
                        if (!messageBuckets[msg]) messageBuckets[msg] = {};
                        messageBuckets[msg][bucket] = (messageBuckets[msg][bucket] || 0) + count;
                    });
                });
            });
        });
        ...
        Plotly.newPlot(histogramDiv, traces, {
            barmode: 'group',
    ...
    function renderTables() {
        ...
        Object.entries(jsonData.nodes).forEach(([node, nodeData]) => {
            ...
            Object.entries(nodeData).forEach(([logType, logTypeData]) => {
                ...
                nodeHtml += `<h4>Log Type: ${logType}</h4>`;
                nodeHtml += '<table><tr><th>Log Message</th><th>First Occurrence</th><th>Last Occurrence</th><th>Count</th></tr>';
                Object.entries(logMessages).forEach(([msg, stats]) => {
                    nodeHtml += `<tr><td>${msg}</td><td>${stats.StartTime||''}</td><td>${stats.EndTime||''}</td><td>${stats.count||0}</td></tr>`;
                });
                nodeHtml += '</table>';
            });
            if (nodeHtml) {
